//! cx-strategy — the strategy squadron: three built-in bar-driven strategies
//! plus bus-wide signal fusion.
//!
//! One runtime task reacts to COMPLETE M1 bars per configured symbol and
//! evaluates the built-ins ("momentum_x", "meanrev_z", "breakout_d") over a
//! rolling window via [`cx_ta`]. A second task fuses every non-"fusion"
//! signal on the bus — including the LLM strategist's — into one
//! `EngineEvent::Signal{strategy:"fusion"}` opinion per symbol.
//!
//! Invariants:
...
/// The three built-in strategies, in evaluation order. Indices into
/// `Shared::enabled` and `SymState::last_pub` follow this array.
pub(crate) const BUILT_INS: [&str; 3] = ["momentum_x", "meanrev_z", "breakout_d"];
...
pub(crate) struct Shared {
    ...
    pub(crate) enabled: [AtomicBool; 3],
...
impl StrategyHandle {
    ...
    /// Returns false for unknown names (only the three built-ins are known).
    ...
    /// The three built-in strategy names, in evaluation order.
...
mod tests {
    ...
    async fn handle_names_and_unknown_toggle() {
        ...
        assert_eq!(handle.names(), vec!["momentum_x", "meanrev_z", "breakout_d"]);
